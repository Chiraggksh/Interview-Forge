...
export const useAuth = () => {
    ...
    const handleLogin = async ({ email, password }) => {
        setLoading(true) //this setloading functionality is handled by hooks only, in ui u just show the loading phase thats it
        try {
            const data = await login({ email, password })
            setUser(data.user) //which contains id,username and email from user (refer to backend)
        } catch (err) {
